...
class TokensStore:
    ...
    def remove(self, index):
        ...
        client = db.get_client()
        response = client.table("tokens").select("id").order("id").execute()
        rows = response.data if response.data else []
        if not (0 <= index < len(rows)):
            raise IndexError(f"Token index out of range: {index}")
        client.table("tokens").delete().eq("id", rows[index]["id"]).execute()

    # --- internals ---

    def _read_local(self):
        client = db.get_client()
        response = client.table("tokens").select("payload").order("id").execute()
        rows = response.data if response.data else []
        out = []
        for r in rows:
            try:
                # Supabase returns JSONB as dict directly, no need to json.loads
                payload = r["payload"]
